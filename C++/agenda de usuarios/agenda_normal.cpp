...
int main(void){
    ...
    if(elec==2){
        printf("\nDime el nombre del archivo (con extension .txt): ");
        scanf("%s",&archivo);
...
void llegir(char x[20]){
    ...
    if (fichero==NULL){
        ...
        while(fscanf(fichero,"Nombre: %s\nPrimer apellido: %s\nSegundo apellido: %s\nDireccion: %s\nTelefono: %i\nFecha nacimiento: %i/%i/%i\n\n",
        &persona.nom, &persona.cognom1, &persona.cognom2, &persona.direccio, &persona.telefon, &persona.fecha.dia, &persona.fecha.mes, &persona.fecha.any)!=EOF){
    ...
    if(fichero!=NULL){
        ...
        while(conti!=2){
            ...
            while((persona.telefon<=0)||(persona.telefon>999999999)){
                printf("Introduzca su telefono (maximo 9 cifras): ");
